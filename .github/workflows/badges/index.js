...
const packageManifest = require(path.join(__dirname, "../../../module.json"));
...
// /** @type {PackageData} */
const packageInfo = packageManifest;
...
function getBadge(packageInfo) {
  ...
  // Deprecated
  if (packageInfo.deprecated) return getDeprecated(packageInfo, data);
  ...
  //   if (!packageInfo.maintained) return getNotMaintained(data);
...
function getDeprecated(packageInfo, data) {
  ...
  data.message = packageInfo.deprecated.coreVersion // If it's a string, display that, otherwise a static message
    ? `After Foundry version ${packageInfo.deprecated.coreVersion}`
...
function getMaintained(packageInfo, data) {
  data.label =
    packageInfo.maintainers.length > 1 ? "Maintainers" : "Maintainer";
